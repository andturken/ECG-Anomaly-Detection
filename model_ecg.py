...
print( (A==-1).mean(),(labels != 0).mean(), ((A==-1)== (labels != 0)).mean())

#%%
from sklearn.decomposition import MiniBatchSparsePCA
X = data_pts_1
mbsp = MiniBatchSparsePCA(n_components=20, alpha=1, ridge_alpha=0.01,batch_size=4, n_jobs=-1)
mbsp.fit(X)
...
mbdl = decomposition.MiniBatchDictionaryLearning(n_jobs=-1,
                                          n_components=20, alpha=0.1,
                                          n_iter=200, batch_size=5,
                                          random_state=0)
mbdl.fit(X)
...
mbdlp = decomposition.MiniBatchDictionaryLearning(n_jobs=-1,
                                          n_components=20, alpha=1,
                                          n_iter=100, batch_size=5,  positive_dict=True,
                                          random_state=0)
mbdlp.fit(X)

#%%
from sklearn.cluster._kmeans import MiniBatchKMeans
mbkm = MiniBatchKMeans( n_clusters=10, tol=1e-3, batch_size=20,
...
nmf = decomposition.NMF(n_components=20, init='nndsvda', tol=5e-3, shuffle=True)
nmf.fit(X)
#%%
from sklearn.decomposition import SparsePCA
X = data_pts_1
transformer = KernelPCA(n_components=20, kernel='linear', n_jobs=1)
transformer.fit(X)
X_transformed = transformer.transform(X)
X_transformed.shape
transformer
...
fica = decomposition.FastICA(n_components=20, whiten=True, algorithm ='parallel')
fica.fit(X)
...
print(classification_report_imbalanced(labels_test, y_pred,     target_names=['N','A','L','R','V']))
